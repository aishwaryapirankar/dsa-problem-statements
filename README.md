...
2. For each number, check if it already exists in the 'seen' set.
3. If found, return 'True'. If the loop finishes, return 'False'.
...
2. For every element at 'i', scan all previous elements at 'j'.
3. If 'nums[i] > nums[j]', update 'lis[i]' to be the maximum of its current value or 'lis[j] + 1'.
